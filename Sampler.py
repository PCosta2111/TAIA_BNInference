...
def Gibbs_Sampling(marginals, joints, trijoints, bayes_net_prob, test_samples, burn_in=10,
                   num_samples=10000, num_features=14, verbose=True):
    """
...
def sample_next_feature(marginals, joints, trijoints, sample_init, feature_num, num_features=14):
    ...
    [sample_feature_value] = np.random.choice(possible_values, 1,
                                              p=list(prob))  # sample now for one.
...
def sample_intilization(marginals, joints, trijoints, num_features=14):
    ...
    while 1:
        for i in range(14):
            ...
            [feature_i_value] = np.random.choice(feature_dict.keys(), 1,
                                                 p=feature_dict.values())  # pick a value based on pdf
...
for i in range(1, 11):
    error = 0.0
    for j in range(i):
        error += Gibbs_Sampling(marginals, joints, trijoints, bayes_net_prob, test_samples,
                                burn_in=10, num_samples=20000, num_features=14, verbose=False)
    error /= i
    errors.append(error)
    nums.append(i)
